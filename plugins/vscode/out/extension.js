...
function activate(context) {
    ...
    const completionProvider = vscode.languages.registerCompletionItemProvider(selector, {
        async provideCompletionItems(document, position) {
            ...
            if (match) {
                ...
                const replaceRange = new vscode.Range(position.line, position.character - typedLength, position.line, position.character);
                try {
    ...
    // 注册悬停提示
    const hoverProvider = vscode.languages.registerHoverProvider(['javascript', 'typescript', 'javascriptreact', 'typescriptreact', 'java'], {
            ...
            while ((match = regex.exec(line)) !== null) {
                ...
                if (position.character >= codeStart && position.character <= codeEnd) {
                    try {
                        ...
                        const json = await res.json();
                        const p = json?.data?.find((d) => d.code === code);
